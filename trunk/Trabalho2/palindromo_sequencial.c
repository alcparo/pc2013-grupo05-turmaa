...
int main(int argc, char * argv[]){
	...
	/* Declaracao de variaveis */
	int wordPalindromeCount, phrasePalindromeCount; // Irao armazenar a quantidade total de palindromos por palavra e por frase.
	int* wordPrimeCount = (int*)malloc(sizeof(int));	*wordPrimeCount = 0; // Declaracao, alocacao e inicializacao de variavel que ira armazenar a quantidade de palindromos de palavra primos (soma do codigo ASCII).
	int* phrasePrimeCount = (int*)malloc(sizeof(int));	*phrasePrimeCount = 0; // Declaracao, alocacao e inicializacao de variavel que ira armazenar a quantidade de palindromos de frase primos  (soma do codigo ASCII).
	
	/* Chamada de funcao */ 
	wordPalindromeCount = wordPalindrome(buffer, argv[1], wordPrimeCount);
	phrasePalindromeCount = phrasePalindrome(buffer, argv[1], phrasePrimeCount);	
